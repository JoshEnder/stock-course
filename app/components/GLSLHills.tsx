...
const GLSLHills = ({
  ...
  // Keep a ref in sync with the prop so the render loop always sees the
  // current value without needing to re-initialise the whole Three.js scene.
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      // Scroll-driven camera flight into the mountains
      const sp = scrollProgressRef.current;
      camera.position.z = cameraZ - (sp * 80);
      camera.position.y = 16 + (sp * 8);
      camera.lookAt(new THREE.Vector3(0, 28 + sp * 10, 0));
